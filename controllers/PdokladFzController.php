...
use app\models\PdokladFzSearch;
 ...
 */
class PdokladFzController extends Controller
{
	...
	{
		$searchModel = new PdokladFzSearch();
	...
	public function actionUpdate($id)
	{
		...
		if ($model->load(Yii::$app->request->post()))
		{
			$pdoklad = PdokladFz::findOne($id);
			$old_castka = $pdoklad->castka;
			...
			if ($save)
			{
				...
				// Log
				$addLog = Log::addLog("Opravil pokladní doklad", $model->id);
				Yii::$app->session->setFlash('success', "Pokladní doklad opraven úspěšně");
			}
			return $this->redirect(['index']);
		} else
		{
	...
	public function actionDelete($id)
	{
		...
		// Log
		$addLog = Log::addLog("Smazal pokladní doklad", $id);
		Yii::$app->session->setFlash('success', "Pokladní doklad smazan úspěšně");
	...
	{
		if (($model = PdokladFz::findOne($id)) !== null)
		{
			return $model;
		} else
		{
  ...
  <tr>
	...
	<td valign="top">
		...
		www.dvere-erkado.cz
			...
			'filename' => "Pokladni-doklad-{$id}.pdf",
			'content' => $content,
		]);
