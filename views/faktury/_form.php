...
<?php $form = ActiveForm::begin(); ?>
	...
	<div class="form-group field-faktury-vystaveno required col-xs-2">
		<?php
	...
	<div class="form-group field-nabidky-vystaveno required col-xs-2">
		<?
			if (Yii::$app->controller->action->id == 'create')
			{
				$zaloha = FakturyZalohove::find()->where(['nabidky_id' => $model->nabidky_id])->sum('celkem');
			} 
			else
			{
				$zaloha = $model->zaloha;
			}
			echo $form->field($model, 'zaloha')->textInput(['maxlength' => true, 'value' => $zaloha]) ?>
	</div>
	...
	<div class="form-group field-nabidky-vystaveno required col-xs-2">
		<?
			if (Yii::$app->controller->action->id == 'create')
			{
				$zaloha_dph = FakturyZalohove::find()->where(['nabidky_id' => $model->nabidky_id])->sum('celkem_dph');
			} 
			else
			{
				$zaloha = $model->zaloha_dph;
			}
			echo $form->field($model, 'zaloha_dph')->textInput(['maxlength' => true, 'value' => $zaloha]) ?>
	</div>
	...
	<? 
		if ($model->id > 0)
		{
			$countNS = FakturySeznam::getCountFakturySeznam($model->id);
		}
		else if($nab['id'] > 0)
		{
...
if ($model->id > 0 || $nab['id'] > 0)
{
	if($model->id > 0)
	{
		$dataNS = FakturySeznam::getFakturySeznam($model->id);
	}
	else if ($nab['id'])
	{
...
?>
    ...
    <div class="form-group col-xs-12">
		<?= Html::a('Zpět', ['/faktury/index'], ['class'=>'btn btn-success btn-100']) ?>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
