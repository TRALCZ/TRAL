...
<div class="faktury-prijate-form">
...
<?php $form = ActiveForm::begin(); ?>
	...
	<?
		...
		{
			$objednavky_id = $model->objednavky_id;
		}
	?>
	<?= $form->field($model, 'objednavky_id')->hiddenInput(['value' => $objednavky_id])->label(false); ?>
	...
	<div class="form-group field-faktury-prijate-vystaveno required col-xs-2">
		<?php
			if (Yii::$app->controller->action->id == 'create')
			{
				if($obj['id'] > 0)
				{
					$vystaveno = date("d.m.Y", strtotime($obj['vystaveno']));
				}
				else
				{
	...
	<div class="form-group field-faktury-prijate-vystaveno required col-xs-2">
		<?
			if (Yii::$app->controller->action->id == 'create')
			{
				if($obj['id'] > 0)
				{
					$platnost = date("d.m.Y", strtotime($obj['platnost']));
				}
				else
				{
	...
	<div class="form-group field-modely required col-xs-2">
		<?
			if (Yii::$app->controller->action->id == 'create')
			{
				$zpusoby_platby_id = $obj['zpusoby_platby_id'];
			} 
			else
			{
	...
	<div style="float: left; display: block;" class="col-xs-6">
		...
		if (Yii::$app->controller->action->id == 'create')
		{
			$zakazniky = empty($obj['zakazniky_id']) ? '' : Zakazniky::findOne($obj['zakazniky_id'])->name;
			$zakazniky_id = $obj['zakazniky_id'];
		} 
		else
		{
	...
	<? 
		if ($model->id > 0)
		{
			$countNS = FakturyPrijateSeznam::getCountFakturyPrijateSeznam($model->id);
		}
		else if($obj['id'] > 0)
		{
			$countNS = ObjednavkySeznam::getCountObjednavkySeznam($obj['id']);
		}
	?>

	<div class="form-group col-xs-12" >
		<label class="control-label" for="nabidky-odberatel_id">Položky vystavené faktury</label> <span id="add_nabidka_polozka" data-count="<?=$countNS+1?>">Přidat položku</span>
		<div class="nabidka-polozky" id="nabidka_polozky">
			...
			<!-- Update -->
<?
if ($model->id > 0 || $obj['id'] > 0)
{
	if($model->id > 0)
	{
		$dataNS = FakturyPrijateSeznam::getFakturyPrijateSeznam($model->id);
	}
	else if ($obj['id'])
	{
		$dataNS = ObjednavkySeznam::getObjednavkySeznam($obj['id']);
	}
...
?>
    ...
    <div class="form-group col-xs-12">
		<?= Html::a('Zpět', ['/faktury-prijate/index'], ['class'=>'btn btn-success btn-100']) ?>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
