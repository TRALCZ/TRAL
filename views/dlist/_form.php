...
<?php $form = ActiveForm::begin(); ?>
	...
	<div class="form-group field-dlist-vystaveno required col-xs-2">
		<?php
			if (Yii::$app->controller->action->id == 'create')
			{
				if($nab['id'] > 0)
	...
	<? 
		if ($model->id > 0)
		{
			$countNS = DlistSeznam::getCountDlistSeznam($model->id);
		}
		else if($nab['id'] > 0)
		{
...
if ($model->id > 0 || $nab['id'] > 0)
{
	if($model->id > 0)
	{
		$dataNS = DlistSeznam::getDlistSeznam($model->id);
	}
	else if ($nab['id'])
	{
...
?>
    ...
    <div class="form-group col-xs-12">
		<?
			...
			{
				echo Html::a('Zpět', ['/dlist/index'], ['class'=>'btn btn-success btn-100']);
				echo '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp';
				echo Html::submitButton($model->isNewRecord ? 'Uložít' : 'Uložít', ['class' => $model->isNewRecord ? 'btn btn-primary btn-100' : 'btn btn-primary btn-100', id=>'btn_submit']);
			}
		?>
